...
void transferMaterialsToGPU()
{
    ...
    if (materials != NULL) {
...
#if PALPHA_POROSITY
        cudaVerify(hipMalloc((void **)&matporjutzi_p_elastic_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporjutzi_p_transition_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporjutzi_p_compacted_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporjutzi_alpha_0_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporjutzi_alpha_e_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporjutzi_alpha_t_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporjutzi_n1_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporjutzi_n2_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matcs_porous_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matcs_solid_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matcrushcurve_style_d, numberOfElements*sizeof(int)));
#endif
#if VARIABLE_SML
        cudaVerify(hipMalloc((void **)&mat_f_sml_max_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&mat_f_sml_min_d, numberOfElements*sizeof(double)));
#endif
#if SIRONO_POROSITY
        cudaVerify(hipMalloc((void **)&matporsirono_K_0_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporsirono_rho_0_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporsirono_rho_s_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporsirono_gamma_K_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporsirono_alpha_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporsirono_pm_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporsirono_phimax_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporsirono_phi0_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporsirono_delta_d, numberOfElements*sizeof(double)));
#endif
#if EPSALPHA_POROSITY
        cudaVerify(hipMalloc((void **)&matporepsilon_kappa_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporepsilon_alpha_0_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporepsilon_epsilon_e_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporepsilon_epsilon_x_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matporepsilon_epsilon_c_d, numberOfElements*sizeof(double)));
#endif
#if NAVIER_STOKES
        cudaVerify(hipMalloc((void **)&matnu_d, numberOfElements*sizeof(double)));
	    cudaVerify(hipMalloc((void **)&matalpha_shakura_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matzeta_d, numberOfElements*sizeof(double)));
#endif
#if ARTIFICIAL_STRESS
        cudaVerify(hipMalloc((void **)&matexponent_tensor_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matepsilon_stress_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matmean_particle_distance_d, numberOfElements*sizeof(double)));
#endif
#if LOW_DENSITY_WEAKENING
        cudaVerify(hipMalloc((void **)&matLdwEtaLimit_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matLdwAlpha_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matLdwBeta_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matLdwGamma_d, numberOfElements*sizeof(double)));
#endif
        //begin of ANEOS allocations in (global) device memory (everything linearized)
        cudaVerify(hipMalloc((void **)&aneos_n_rho_d, numberOfElements*sizeof(int)));
        cudaVerify(hipMalloc((void **)&aneos_n_e_d, numberOfElements*sizeof(int)));
        cudaVerify(hipMalloc((void **)&aneos_bulk_cs_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&aneos_rho_d, run_aneos_rho_id*sizeof(double)));
        cudaVerify(hipMalloc((void **)&aneos_e_d, run_aneos_e_id*sizeof(double)));
        cudaVerify(hipMalloc((void **)&aneos_p_d, run_aneos_matrix_id*sizeof(double)));
        cudaVerify(hipMalloc((void **)&aneos_cs_d, run_aneos_matrix_id*sizeof(double)));
        cudaVerify(hipMalloc((void **)&aneos_rho_id_d, numberOfElements*sizeof(int)));
        cudaVerify(hipMalloc((void **)&aneos_e_id_d, numberOfElements*sizeof(int)));
        cudaVerify(hipMalloc((void **)&aneos_matrix_id_d, numberOfElements*sizeof(int)));
        //end of ANEOS allocations in (global) device memory
        cudaVerify(hipMalloc((void **)&matSml_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matnoi_d, numberOfElements*sizeof(int)));
        cudaVerify(hipMalloc((void **)&matEOS_d, numberOfElements*sizeof(int)));
        cudaVerify(hipMalloc((void **)&matPolytropicK_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matPolytropicGamma_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matAlpha_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matBeta_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matBulkmodulus_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matShearmodulus_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matYieldStress_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matInternalFriction_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matInternalFrictionDamaged_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matRho0_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matIsothermalSoundSpeed_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matTillRho0_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matTillE0_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matTillEiv_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matTillEcv_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matTilla_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matTillb_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matTillA_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matTillB_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matTillAlpha_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matTillBeta_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matcsLimit_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matRhoLimit_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matN_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matCohesion_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matCohesionDamaged_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matFrictionAngle_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matFrictionAngleDamaged_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matAlphaPhi_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matCohesionCoefficient_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matMeltEnergy_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matDensityFloor_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matEnergyFloor_d, numberOfElements*sizeof(double)));
#if JC_PLASTICITY
        cudaVerify(hipMalloc((void **)&matjc_y0_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matjc_B_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matjc_n_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matjc_m_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matjc_edot0_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matjc_C_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matjc_Tref_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matjc_Tmelt_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matCp_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMalloc((void **)&matCV_d, numberOfElements*sizeof(double)));
#endif
        cudaVerify(hipMalloc((void **)&matdensity_via_kernel_sum_d, numberOfElements*sizeof(int)));
        cudaVerify(hipMemcpy(matdensity_via_kernel_sum_d, density_via_kernel_sum, numberOfElements*sizeof(int), hipMemcpyHostToDevice));
#if SOLID
        cudaVerify(hipMalloc((void **)&matYoungModulus_d, numberOfElements*sizeof(double)));
        cudaVerify(hipMemcpy(matYoungModulus_d, young_modulus, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matYoungModulus), &matYoungModulus_d, sizeof(void*)));
#endif

        hipGetSymbolAddress((void **)&pc_pointer, gravConst);
        hipMemcpy(pc_pointer, &grav_const, sizeof(double), hipMemcpyHostToDevice);

        hipGetSymbolAddress((void **)&pc_pointer, scale_height);
        hipMemcpy(pc_pointer, &scale_height_host, sizeof(double), hipMemcpyHostToDevice);

        /* predictor corrector integration parameters are set in symbol memory */
        hipGetSymbolAddress((void **)&pc_pointer, Smin_d);
        hipMemcpy(pc_pointer, &Smin, sizeof(double), hipMemcpyHostToDevice);
        hipGetSymbolAddress((void **)&pc_pointer, emin_d);
        hipMemcpy(pc_pointer, &emin, sizeof(double), hipMemcpyHostToDevice);
        hipGetSymbolAddress((void **)&pc_pointer, rhomin_d);
        hipMemcpy(pc_pointer, &rhomin, sizeof(double), hipMemcpyHostToDevice);
        hipGetSymbolAddress((void **)&pc_pointer, damagemin_d);
        hipMemcpy(pc_pointer, &damagemin, sizeof(double), hipMemcpyHostToDevice);
        hipGetSymbolAddress((void **)&pc_pointer, alphamin_d);
        hipMemcpy(pc_pointer, &alphamin, sizeof(double), hipMemcpyHostToDevice);
        hipGetSymbolAddress((void **)&pc_pointer, betamin_d);
        hipMemcpy(pc_pointer, &betamin, sizeof(double), hipMemcpyHostToDevice);
        hipGetSymbolAddress((void **)&pc_pointer, alpha_epspormin_d);
        hipMemcpy(pc_pointer, &alpha_epspormin, sizeof(double), hipMemcpyHostToDevice);
        hipGetSymbolAddress((void **)&pc_pointer, epsilon_vmin_d);
        hipMemcpy(pc_pointer, &epsilon_vmin, sizeof(double), hipMemcpyHostToDevice);

#if PALPHA_POROSITY
        hipGetSymbolAddress((void **)&pc_pointer, max_abs_pressure_change);
        hipMemcpy(pc_pointer, &max_abs_pressure_change_host, sizeof(double), hipMemcpyHostToDevice);
        ...
        cudaVerify(hipMemcpy(matSml_d, sml, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
#if VARIABLE_SML
        cudaVerify(hipMemcpy(mat_f_sml_max_d, f_sml_max , numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(mat_f_sml_min_d, f_sml_min , numberOfElements*sizeof(double), hipMemcpyHostToDevice));
#endif
        cudaVerify(hipMemcpy(matnoi_d, noi, numberOfElements*sizeof(int), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matEOS_d, eos, numberOfElements*sizeof(int), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matPolytropicK_d, polytropic_K, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matPolytropicGamma_d, polytropic_gamma, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
  	    cudaVerify(hipMemcpy(matAlpha_d, alpha, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matBeta_d, beta, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matBulkmodulus_d, bulk_modulus, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
#if NAVIER_STOKES
        cudaVerify(hipMemcpy(matnu_d, nu, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
  	    cudaVerify(hipMemcpy(matalpha_shakura_d, alpha_shakura, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matzeta_d, zeta, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
#endif
        cudaVerify(hipMemcpy(matShearmodulus_d, shear_modulus, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matYieldStress_d, yield_stress, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matInternalFriction_d, internal_friction, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matInternalFrictionDamaged_d, internal_friction_damaged, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matRho0_d, rho_0, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matIsothermalSoundSpeed_d, isothermal_cs, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matTillRho0_d, till_rho_0, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matTillE0_d, till_E_0, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matTillEcv_d, till_E_cv, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matTillEiv_d, till_E_iv, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matTilla_d, till_a, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matTillb_d, till_b, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matTillA_d, till_A, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matTillB_d, till_B, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matTillAlpha_d, till_alpha, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matTillBeta_d, till_beta, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matcsLimit_d, csLimit, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matRhoLimit_d, rho_limit, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matN_d, n, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matCohesion_d, cohesion, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matCohesionDamaged_d, cohesion_damaged, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matFrictionAngle_d, friction_angle, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matFrictionAngleDamaged_d, friction_angle_damaged, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matAlphaPhi_d, alpha_phi, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matCohesionCoefficient_d, cohesion_coefficient, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matMeltEnergy_d, melt_energy, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matDensityFloor_d, density_floor, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matEnergyFloor_d, energy_floor, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        //begin copying ANEOS data from host to (global) device memory
        cudaVerify(hipMemcpy(aneos_n_rho_d, g_aneos_n_rho, numberOfElements*sizeof(int), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(aneos_n_e_d, g_aneos_n_e, numberOfElements*sizeof(int), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(aneos_bulk_cs_d, g_aneos_bulk_cs, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        for (i=0; i<numberOfMaterials; i++) {
            if (eos[i] == EOS_TYPE_ANEOS  ||  eos[i] == EOS_TYPE_JUTZI_ANEOS) {
                cudaVerify(hipMemcpy(aneos_rho_d+aneos_rho_id[i], g_aneos_rho[i], g_aneos_n_rho[i]*sizeof(double), hipMemcpyHostToDevice));
                cudaVerify(hipMemcpy(aneos_e_d+aneos_e_id[i], g_aneos_e[i], g_aneos_n_e[i]*sizeof(double), hipMemcpyHostToDevice));
                for(j=0; j<g_aneos_n_rho[i]; j++) {
                    cudaVerify(hipMemcpy(aneos_p_d+aneos_matrix_id[i]+j*g_aneos_n_e[i], g_aneos_p[i][j], g_aneos_n_e[i]*sizeof(double), hipMemcpyHostToDevice));
                    cudaVerify(hipMemcpy(aneos_cs_d+aneos_matrix_id[i]+j*g_aneos_n_e[i], g_aneos_cs[i][j], g_aneos_n_e[i]*sizeof(double), hipMemcpyHostToDevice));
                }
            }
        }
        cudaVerify(hipMemcpy(aneos_rho_id_d, aneos_rho_id, numberOfElements*sizeof(int), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(aneos_e_id_d, aneos_e_id, numberOfElements*sizeof(int), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(aneos_matrix_id_d, aneos_matrix_id, numberOfElements*sizeof(int), hipMemcpyHostToDevice));
        ...
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(aneos_n_rho_c), &aneos_n_rho_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(aneos_n_e_c), &aneos_n_e_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(aneos_bulk_cs_c), &aneos_bulk_cs_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(aneos_rho_c), &aneos_rho_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(aneos_e_c), &aneos_e_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(aneos_p_c), &aneos_p_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(aneos_cs_c), &aneos_cs_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(aneos_rho_id_c), &aneos_rho_id_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(aneos_e_id_c), &aneos_e_id_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(aneos_matrix_id_c), &aneos_matrix_id_d, sizeof(void*)));
        //end copying pointers to constant device memory
#if JC_PLASTICITY
        cudaVerify(hipMemcpy(matjc_y0_d, jc_y0, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matjc_B_d, jc_B, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matjc_n_d, jc_n, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matjc_m_d, jc_m, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matjc_edot0_d, jc_edot0, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matjc_C_d, jc_C, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matjc_Tref_d, jc_Tref, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matjc_Tmelt_d, jc_Tmelt, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matCp_d, Cp, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matCV_d, CV, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
#endif
#if NAVIER_STOKES
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matnu), &matnu_d, sizeof(void*)));
  	    cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matalpha_shakura), &matalpha_shakura_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matzeta), &matzeta_d, sizeof(void*)));
#endif
#if ARTIFICIAL_STRESS
        cudaVerify(hipMemcpy(matexponent_tensor_d, exponent_tensor, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matepsilon_stress_d, epsilon_stress, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matmean_particle_distance_d, mean_particle_distance, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matexponent_tensor), &matexponent_tensor_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matepsilon_stress), &matepsilon_stress_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matmean_particle_distance), &matmean_particle_distance_d, sizeof(void*)));
#endif
#if LOW_DENSITY_WEAKENING
        cudaVerify(hipMemcpy(matLdwEtaLimit_d, ldw_eta_limit, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matLdwAlpha_d, ldw_alpha, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matLdwBeta_d, ldw_beta, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matLdwGamma_d, ldw_gamma, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matLdwEtaLimit), &matLdwEtaLimit_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matLdwAlpha), &matLdwAlpha_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matLdwBeta), &matLdwBeta_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matLdwGamma), &matLdwGamma_d, sizeof(void*)));
#endif
#if PALPHA_POROSITY
        cudaVerify(hipMemcpy(matporjutzi_p_elastic_d, porjutzi_p_elastic, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporjutzi_p_transition_d, porjutzi_p_transition, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporjutzi_p_compacted_d, porjutzi_p_compacted, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporjutzi_alpha_0_d, porjutzi_alpha_0, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporjutzi_alpha_e_d, porjutzi_alpha_e, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporjutzi_alpha_t_d, porjutzi_alpha_t, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporjutzi_n1_d, porjutzi_n1, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporjutzi_n2_d, porjutzi_n2, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matcs_porous_d, cs_porous, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matcs_solid_d, cs_solid, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matcrushcurve_style_d, crushcurve_style, numberOfElements*sizeof(int), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporjutzi_p_elastic), &matporjutzi_p_elastic_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporjutzi_p_transition), &matporjutzi_p_transition_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporjutzi_p_compacted), &matporjutzi_p_compacted_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporjutzi_alpha_0), &matporjutzi_alpha_0_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporjutzi_alpha_e), &matporjutzi_alpha_e_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporjutzi_alpha_t), &matporjutzi_alpha_t_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporjutzi_n1), &matporjutzi_n1_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporjutzi_n2), &matporjutzi_n2_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matcs_porous), &matcs_porous_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matcs_solid), &matcs_solid_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matcrushcurve_style), &matcrushcurve_style_d, sizeof(void*)));
#endif
#if SIRONO_POROSITY
        cudaVerify(hipMemcpy(matporsirono_K_0_d, porsirono_K_0, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporsirono_rho_0_d, porsirono_rho_0, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporsirono_rho_s_d, porsirono_rho_s, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporsirono_gamma_K_d, porsirono_gamma_K, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporsirono_alpha_d, porsirono_alpha, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporsirono_pm_d, porsirono_pm, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporsirono_phimax_d, porsirono_phimax, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporsirono_phi0_d, porsirono_phi0, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporsirono_delta_d, porsirono_delta, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporsirono_K_0), &matporsirono_K_0_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporsirono_rho_0), &matporsirono_rho_0_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporsirono_rho_s), &matporsirono_rho_s_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporsirono_gamma_K), &matporsirono_gamma_K_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporsirono_alpha), &matporsirono_alpha_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporsirono_pm), &matporsirono_pm_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporsirono_phimax), &matporsirono_phimax_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporsirono_phi0), &matporsirono_phi0_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporsirono_delta), &matporsirono_delta_d, sizeof(void*)));
#endif
#if EPSALPHA_POROSITY
        cudaVerify(hipMemcpy(matporepsilon_kappa_d, porepsilon_kappa, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporepsilon_alpha_0_d, porepsilon_alpha_0, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporepsilon_epsilon_e_d, porepsilon_epsilon_e, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporepsilon_epsilon_x_d, porepsilon_epsilon_x, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpy(matporepsilon_epsilon_c_d, porepsilon_epsilon_c, numberOfElements*sizeof(double), hipMemcpyHostToDevice));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporepsilon_kappa), &matporepsilon_kappa_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporepsilon_alpha_0), &matporepsilon_alpha_0_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporepsilon_epsilon_e), &matporepsilon_epsilon_e_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporepsilon_epsilon_x), &matporepsilon_epsilon_x_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matporepsilon_epsilon_c), &matporepsilon_epsilon_c_d, sizeof(void*)));
#endif

        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matSml), &matSml_d, sizeof(void*)));
#if VARIABLE_SML
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(mat_f_sml_max), &mat_f_sml_max_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(mat_f_sml_min), &mat_f_sml_min_d, sizeof(void*)));
#endif
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matnoi), &matnoi_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matdensity_via_kernel_sum), &matdensity_via_kernel_sum_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matEOS), &matEOS_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matPolytropicK), &matPolytropicK_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matPolytropicGamma), &matPolytropicGamma_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matAlpha), &matAlpha_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matBeta), &matBeta_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matBulkmodulus), &matBulkmodulus_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matShearmodulus), &matShearmodulus_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matYieldStress), &matYieldStress_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matInternalFriction), &matInternalFriction_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matInternalFrictionDamaged), &matInternalFrictionDamaged_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matRho0), &matRho0_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matIsothermalSoundSpeed), &matIsothermalSoundSpeed_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matTillRho0), &matTillRho0_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matTillE0), &matTillE0_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matTillEiv), &matTillEiv_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matTillEcv), &matTillEcv_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matTilla), &matTilla_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matTillb), &matTillb_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matTillA), &matTillA_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matTillB), &matTillB_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matTillAlpha), &matTillAlpha_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matTillBeta), &matTillBeta_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matcsLimit), &matcsLimit_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matRhoLimit), &matRhoLimit_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matN), &matN_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matCohesion), &matCohesion_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matCohesionDamaged), &matCohesionDamaged_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matFrictionAngle), &matFrictionAngle_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matFrictionAngleDamaged), &matFrictionAngleDamaged_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matAlphaPhi), &matAlphaPhi_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matCohesionCoefficient), &matCohesionCoefficient_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matMeltEnergy), &matMeltEnergy_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matDensityFloor), &matDensityFloor_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matEnergyFloor), &matEnergyFloor_d, sizeof(void*)));
#if JC_PLASTICITY
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matjc_y0), &matjc_y0_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matjc_B), &matjc_B_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matjc_n), &matjc_n_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matjc_m), &matjc_m_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matjc_edot0), &matjc_edot0_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matjc_C), &matjc_C_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matjc_Tref), &matjc_Tref_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matjc_Tmelt), &matjc_Tmelt_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matCp), &matCp_d, sizeof(void*)));
        cudaVerify(hipMemcpyToSymbol(HIP_SYMBOL(matCV), &matCV_d, sizeof(void*)));
#endif
...
void cleanupMaterials()
{
    ...
    cudaVerify(hipFree(aneos_n_rho_d));
    cudaVerify(hipFree(aneos_n_e_d));
    cudaVerify(hipFree(aneos_bulk_cs_d));
    cudaVerify(hipFree(aneos_rho_d));
    cudaVerify(hipFree(aneos_e_d));
    cudaVerify(hipFree(aneos_p_d));
    cudaVerify(hipFree(aneos_cs_d));
    cudaVerify(hipFree(aneos_rho_id_d));
    cudaVerify(hipFree(aneos_e_id_d));
    cudaVerify(hipFree(aneos_matrix_id_d));
    //end freeing of ANEOS (global) device memory
    cudaVerify(hipFree(matSml_d));
    cudaVerify(hipFree(matInternalFriction_d));
    cudaVerify(hipFree(matInternalFrictionDamaged_d));
    cudaVerify(hipFree(matYieldStress_d));
    cudaVerify(hipFree(matnoi_d));
    cudaVerify(hipFree(matEOS_d));
    cudaVerify(hipFree(matPolytropicGamma_d));
    cudaVerify(hipFree(matPolytropicK_d));
    cudaVerify(hipFree(matAlpha_d));
    cudaVerify(hipFree(matAlphaPhi_d));
    cudaVerify(hipFree(matBeta_d));
    cudaVerify(hipFree(matBulkmodulus_d));
    cudaVerify(hipFree(matYoungModulus_d));
#if VARIABLE_SML
    cudaVerify(hipFree(mat_f_sml_max_d));
    cudaVerify(hipFree(mat_f_sml_min_d));
#endif
    cudaVerify(hipFree(matRho0_d));
    cudaVerify(hipFree(matTillRho0_d));
    cudaVerify(hipFree(matTilla_d));
    cudaVerify(hipFree(matTillA_d));
    cudaVerify(hipFree(matTillb_d));
    cudaVerify(hipFree(matTillB_d));
    cudaVerify(hipFree(matTillAlpha_d));
    cudaVerify(hipFree(matTillBeta_d));
    cudaVerify(hipFree(matcsLimit_d));
    cudaVerify(hipFree(matTillE0_d));
    cudaVerify(hipFree(matTillEcv_d));
    cudaVerify(hipFree(matTillEiv_d));
    cudaVerify(hipFree(matRhoLimit_d));
    cudaVerify(hipFree(matShearmodulus_d));
    cudaVerify(hipFree(matN_d));
    cudaVerify(hipFree(matCohesion_d));
    cudaVerify(hipFree(matCohesionDamaged_d));
    cudaVerify(hipFree(matCohesionCoefficient_d));
    cudaVerify(hipFree(matMeltEnergy_d));
    cudaVerify(hipFree(matFrictionAngle_d));
    cudaVerify(hipFree(matFrictionAngleDamaged_d));
    cudaVerify(hipFree(matDensityFloor_d));
    cudaVerify(hipFree(matEnergyFloor_d));
    cudaVerify(hipFree(matLdwEtaLimit_d));
    cudaVerify(hipFree(matLdwAlpha_d));
    cudaVerify(hipFree(matLdwBeta_d));
    cudaVerify(hipFree(matLdwGamma_d));
    cudaVerify(hipFree(matporjutzi_p_elastic_d));
    cudaVerify(hipFree(matporjutzi_p_transition_d));
    cudaVerify(hipFree(matporjutzi_p_compacted_d));
    cudaVerify(hipFree(matporjutzi_alpha_0_d));
    cudaVerify(hipFree(matporjutzi_alpha_e_d));
    cudaVerify(hipFree(matporjutzi_alpha_t_d));
    cudaVerify(hipFree(matporjutzi_n1_d));
    cudaVerify(hipFree(matporjutzi_n2_d));
    cudaVerify(hipFree(matcs_porous_d));
    cudaVerify(hipFree(matcs_solid_d));
    cudaVerify(hipFree(matcrushcurve_style_d));
    cudaVerify(hipFree(matporsirono_K_0_d));
    cudaVerify(hipFree(matporsirono_rho_0_d));
    cudaVerify(hipFree(matporsirono_rho_s_d));
    cudaVerify(hipFree(matporsirono_gamma_K_d));
    cudaVerify(hipFree(matporsirono_alpha_d));
    cudaVerify(hipFree(matporsirono_pm_d));
    cudaVerify(hipFree(matporsirono_phimax_d));
    cudaVerify(hipFree(matporsirono_phi0_d));
    cudaVerify(hipFree(matporsirono_delta_d));
    cudaVerify(hipFree(matporepsilon_kappa_d));
    cudaVerify(hipFree(matporepsilon_alpha_0_d));
    cudaVerify(hipFree(matporepsilon_epsilon_e_d));
    cudaVerify(hipFree(matporepsilon_epsilon_x_d));
    cudaVerify(hipFree(matporepsilon_epsilon_c_d));
    cudaVerify(hipFree(matjc_y0_d));
    cudaVerify(hipFree(matjc_B_d));
    cudaVerify(hipFree(matjc_n_d));
    cudaVerify(hipFree(matjc_m_d));
    cudaVerify(hipFree(matjc_edot0_d));
    cudaVerify(hipFree(matjc_C_d));
    cudaVerify(hipFree(matjc_Tref_d));
    cudaVerify(hipFree(matjc_Tmelt_d));
    cudaVerify(hipFree(matCp_d));
    cudaVerify(hipFree(matCV_d));
    free(sml);
    free(bulk_modulus);
    free(cs_porous);
    free(till_rho_0);
